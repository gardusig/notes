# ⬆️ Phase 2 — It is the next priority

**Upstream:** [`README.md`](README.md)

When the item becomes **the** thing you are doing:

1. **Open the repo** and **pin** the terminal (see [`../craft-issue/maintenance/repo-bootstrap.md`](../craft-issue/maintenance/repo-bootstrap.md)).
2. **Re-read the ticket**; update stale links or acceptance if reality shifted.
3. **Switch mental mode**: you are not “coding yet”—you are **loading context and planning** unless the change is trivial.

**Previous:** [`idea-and-good-enough-ticket.md`](idea-and-good-enough-ticket.md) · **Next:** [`plan-mode-sharpen.md`](plan-mode-sharpen.md)
