...
3. **Next physical step** — every kept item needs a concrete next action, not a label; see **§ Task system** in [priority.md](priority.md).  
4. **Externalize, then close the notebook** — name loops and park them on the right queue ([governance.md — Thoughts vs execution](governance.md#thoughts-vs-execution)); planning is not a substitute for doing the top task.  
5. **Surfaces first** — employer vs personal queues: [Tasks hub — Task surfaces](../README.md#task-surfaces-and-context) and **§ Task selection rule** in [priority.md](priority.md).  
6. **Body clock before backlog** — on personal time, anchor sleep/meals before deep queue triage ([../routine/README.md](../routine/README.md)).  
