...
```mermaid
flowchart TB
  subgraph ringPower [Ring 1 — Electricity]
    subgraph ringDisplay [Ring 2 — External display]
      subgraph ringComputer [Ring 3 — Standard computer]
        subgraph ringManyInstances [Ring 4 — Many parallel instances]
          subgraph ringOneInstance [Ring 5 — One instance you inhabit]
            ringYou["Ring 6 — Your life (one bot / character / thread in this instance)"]
          end
        end
      end
    end
...
flowchart TB
  subgraph gameInstance [Video game instance]
    subgraph theGuys [The guys — overall control whatever you believe]
      subgraph redPill [Red pill matrix]
        subgraph pinkGroup [Pink group — border red and blue]
          bluePill(("Blue pill matrix"))
        end
      end
    end
  end
...
```mermaid
flowchart LR
  gameStruct["life-game-structure.md"]
  lifeFlow["life-flow-judgment.md"]
  lineageDoc["lineage-descent.md"]
