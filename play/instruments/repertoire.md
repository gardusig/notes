# 📜 Repertoire

**What this file is for:** pieces you **learn or perform** on an instrument—what you know, what you’re working on, and how you label it.

**Pair:** [consume/playlists](../../consume/playlists/README.md) is for **listening** (likes, playlists, library habits)—not the same as this file.

**Index:** [instruments/README.md](README.md) lists every title here with jump links; playlist catalog stays in [consume/playlists](../../consume/playlists/README.md).

---

## 📌 How entries are structured

**Instruments** (use these labels only in metadata; comma-separate when a piece uses several):

- Acoustic guitar  
- Electric guitar  
- Bass  
- Piano / keyboard  

**Scope:** Studio songs plus **covers / arrangements** of game and anime music. Your own tempo or “slowed” versions count—say so in the **Arrangement** row (or a bullet under **Parts I know**).

**Layout:** Each title has a **metadata table** first, then **Parts I know** as a **bullet list**. Nest bullets under an instrument when it clarifies who plays what; use a flat list when one instrument carries the whole arrangement.

**Listening / library:** [consume/playlists](../../consume/playlists/README.md) — likes, playlists, and cleanup workflow.

---

## 📌 Led Zeppelin — Stairway to Heaven

| Field | Value |
|--------|--------|
| **Artist / band** | Led Zeppelin |
| **Year** | 1971 |
| **Album** | *Led Zeppelin IV* |
| **Instruments** | Acoustic guitar, electric guitar |

### 📌 Parts I know

- **Acoustic guitar**
  - Intro / picking sections — *(fill in)*; famous progression; lock tempo
  - Soft vocal sections — *(fill in)*
- **Electric guitar**
  - Heavier sections — *(fill in)*
  - Solo — not learned

---

## 📌 Iron Maiden — Fear of the Dark

| Field | Value |
|--------|--------|
| **Artist / band** | Iron Maiden |
| **Year** | 1992 |
| **Album** | *Fear of the Dark* |
| **Instruments** | Acoustic guitar, electric guitar |

### 📌 Parts I know

- **Electric guitar**
  - Main riffs / rhythm — *(fill in)*; often arranged for one guitar
  - Clean / arpeggiated parts — *(fill in)*
  - Solo — not learned

---

## 📌 Guns N’ Roses — Sweet Child O’ Mine

| Field | Value |
|--------|--------|
| **Artist / band** | Guns N’ Roses |
| **Year** | 1987 |
| **Album** | *Appetite for Destruction* |
| **Instruments** | Acoustic guitar, electric guitar |

### 📌 Parts I know

- **Electric guitar**
  - Overall piece — rough / sketchy; whole song still shaky
  - Intro / signature riff — *(fill in)*
  - Verse / chorus rhythm — *(fill in)*
  - Solo — not learned

---

## 📌 Guns N’ Roses — Paradise City

| Field | Value |
|--------|--------|
| **Artist / band** | Guns N’ Roses |
| **Year** | 1987 |
| **Album** | *Appetite for Destruction* |
| **Instruments** | Acoustic guitar, electric guitar |

### 📌 Parts I know

- **Electric guitar**
  - Intro / main riffs — *(fill in)*
  - Verse / chorus — *(fill in)*
  - Solo — not learned

---

## 🎸 Flaherty Brotherhood — Death Valley Blues

| Field | Value |
|--------|--------|
| **Artist / band** | Flaherty Brotherhood |
| **Year** | *(confirm on your copy—recent singles/EPs; discography moves fast)* |
| **Album / single** | *(fill if you know)* |
| **Instruments** | Acoustic guitar, electric guitar |

### 📌 Parts I know

- **Electric guitar** *(or acoustic—note which you use)*
  - *(structure)* — *(fill in)*; less known catalog—label sections as you like

---

## 📌 Flaherty Brotherhood — Valley Heat

| Field | Value |
|--------|--------|
| **Artist / band** | Flaherty Brotherhood |
| **Year** | *(may appear as part of “Death Valley Blues” series, e.g. Vol. 2—confirm your track listing)* |
| **Album / single** | *(fill if you know)* |
| **Instruments** | Acoustic guitar, electric guitar |

### 📌 Parts I know

- **Electric guitar** *(or acoustic—note which you use)*
  - *(structure)* — *(fill in)*

---

## 📌 Flaherty Brotherhood — The Castle

| Field | Value |
|--------|--------|
| **Artist / band** | Flaherty Brotherhood |
| **Year** | *(fill in)* |
| **Album / single** | *(fill in)* |
| **Instruments** | Acoustic guitar, electric guitar |

### 📌 Parts I know

- **Electric guitar** *(or acoustic—note which you use)*
  - *(structure)* — *(fill in)*

---

## 📌 Khruangbin — August 10

| Field | Value |
|--------|--------|
| **Artist / band** | Khruangbin |
| **Year** | 2018 |
| **Album** | *Con Todo El Mundo* |
| **Instruments** | Electric guitar, bass *(band is guitar + bass + drums—note what you actually play)* |

### 📌 Parts I know

- **Electric guitar**
  - Main theme / groove — *(fill in)*; good for feel & tone practice
  - Harmonies / layers — *(fill in)*; often simplified to one guitar live
- **Bass** — *(fill in if you learn the line)*

---

## 📌 Muse — Bliss

| Field | Value |
|--------|--------|
| **Artist / band** | Muse |
| **Year** | 2001 |
| **Album** | *Origin of Symmetry* |
| **Instruments** | Piano / keyboard, electric guitar |
| **Notes** | Piano-led on record; guitar covers arrange to guitar—optionally track keys separately |

### 📌 Parts I know

- **Piano / keyboard**
  - Intro / arpeggios — *(fill in)*; as on record or adapted
- **Electric guitar**
  - Verse / chorus (guitar arrangement) — *(fill in)*
  - Bridge / dynamics — *(fill in)*

---

## 🇯🇵 Video game & anime arrangements

Short headers use **source title — track** so *SMW* vs *Mario 64* stay distinct.

### 📌 Super Mario World — Bowser theme

| Field | Value |
|--------|--------|
| **Source** | *Super Mario World* (SNES) |
| **Composer** | Koji Kondo |
| **Year** | 1990 |
| **Arrangement** | Guitar cover / tab *(note which version you use)* |
| **Instruments** | Acoustic guitar, electric guitar |

### 📌 Parts I know

- **Electric guitar** *(or acoustic—note which)*
  - Main theme — *(fill in)*

---

### 📌 Super Mario World — Ending theme

| Field | Value |
|--------|--------|
| **Source** | *Super Mario World* — staff roll / ending |
| **Composer** | Koji Kondo |
| **Year** | 1990 |
| **Arrangement** | Guitar cover / tab |
| **Instruments** | Acoustic guitar, electric guitar |

### 📌 Parts I know

- **Electric guitar** *(or acoustic—note which)*
  - Full piece — *(fill in)*

---

### 📌 Super Mario World — Bowser castle theme

| Field | Value |
|--------|--------|
| **Source** | *Super Mario World* — castle / fortress cue *(separate from final Bowser battle in your notes)* |
| **Composer** | Koji Kondo |
| **Year** | 1990 |
| **Arrangement** | Guitar cover / tab |
| **Instruments** | Acoustic guitar, electric guitar |

### 📌 Parts I know

- **Electric guitar** *(or acoustic—note which)*
  - Main theme — *(fill in)*

---

### 📌 Super Mario 64 — Bowser theme

| Field | Value |
|--------|--------|
| **Source** | *Super Mario 64* — Bowser battles |
| **Composer** | Koji Kondo |
| **Year** | 1996 |
| **Arrangement** | Guitar cover / tab |
| **Instruments** | Acoustic guitar, electric guitar |

### 📌 Parts I know

- **Electric guitar** *(or acoustic—note which)*
  - Main riff / sections — *(fill in)*

---

### 📌 Super Mario 64 — Slide (Rainbow Ride)

| Field | Value |
|--------|--------|
| **Source** | *Super Mario 64* — slippery / slide cue on Rainbow Ride |
| **Composer** | Koji Kondo |
| **Year** | 1996 |
| **Arrangement** | Guitar cover / tab |
| **Instruments** | Acoustic guitar, electric guitar |

### 📌 Parts I know

- **Electric guitar** *(or acoustic—note which)*
  - Full piece — *(fill in)*

---

### 📌 Cuphead — Floral Fury

| Field | Value |
|--------|--------|
| **Source** | *Cuphead* — Cagney Carnation / floral boss |
| **Composer** | Kristofer Maddigan |
| **Year** | 2017 |
| **Arrangement** | Guitar cover / tab |
| **Instruments** | Acoustic guitar, electric guitar |

### 📌 Parts I know

- **Electric guitar** *(or acoustic—note which)*
  - Main sections — *(fill in)*; busy big-band idiom on OST

---

### 🍥 Naruto — Sadness and Sorrow

| Field | Value |
|--------|--------|
| **Source** | *Naruto* anime OST |
| **Composer** | Toshiro Masuda |
| **Year** | 2002 *(anime; OST release may differ)* |
| **Arrangement** | Guitar cover / tab |
| **Instruments** | Acoustic guitar, electric guitar |

### 📌 Parts I know

- **Acoustic guitar** *(typical for this cue—change if you play it on electric)*
  - Melody / accompaniment — *(fill in)*

---

### 🍥 Naruto — Grief and Sorrow

| Field | Value |
|--------|--------|
| **Source** | *Naruto* anime OST *(distinct from “Sadness and Sorrow”)* |
| **Composer** | Toshiro Masuda |
| **Year** | 2002 *(approx.; confirm track)* |
| **Arrangement** | Guitar cover / tab |
| **Instruments** | Acoustic guitar, electric guitar |

### 📌 Parts I know

- **Acoustic guitar** *(or electric—note which)*
  - Full piece — *(fill in)*

---

### 🍥 Naruto — Main theme (slowed)

| Field | Value |
|--------|--------|
| **Source** | *Naruto* — main theme, **slowed** arrangement |
| **Composer** | Toshiro Masuda |
| **Year** | 2002 *(original)* |
| **Arrangement** | Your tempo / backing *(YouTube “slowed” style, etc.)* |
| **Instruments** | Acoustic guitar, electric guitar |

### 📌 Parts I know

- **Electric guitar** *(or acoustic—note which)*
  - Full piece — *(fill in)*

---

### 🍥 Naruto — Main theme

| Field | Value |
|--------|--------|
| **Source** | *Naruto* anime — main theme |
| **Composer** | Toshiro Masuda |
| **Year** | 2002 |
| **Arrangement** | Guitar cover / tab |
| **Instruments** | Acoustic guitar, electric guitar |

### 📌 Parts I know

- **Electric guitar** *(or acoustic—note which)*
  - Full piece — *(fill in)*

---

### 🍥 Naruto — Pain theme

| Field | Value |
|--------|--------|
| **Source** | *Naruto* anime OST — Pain / Akatsuki-related cue *(confirm exact OST title you use)* |
| **Composer** | Toshiro Masuda *(if your version is Shippuden-era, composer may differ—note which)* |
| **Year** | *(fill in: original series vs Shippuden)* |
| **Arrangement** | Guitar cover / tab |
| **Instruments** | Acoustic guitar, electric guitar |

### 📌 Parts I know

- **Electric guitar** *(or acoustic—note which)*
  - Full piece — *(fill in)*

---

### 🍥 Naruto — Sasuke theme

| Field | Value |
|--------|--------|
| **Source** | *Naruto* anime OST — Sasuke character theme *(several cues exist; name the one you play)* |
| **Composer** | Toshiro Masuda |
| **Year** | *(fill)* |
| **Arrangement** | Guitar cover / tab |
| **Instruments** | Acoustic guitar, electric guitar |

### 📌 Parts I know

- **Electric guitar** *(or acoustic—note which)*
  - Full piece — *(fill in)*

---

### 🍥 Naruto — Orochimaru theme

| Field | Value |
|--------|--------|
| **Source** | *Naruto* anime OST — Orochimaru cue |
| **Composer** | Toshiro Masuda |
| **Year** | *(fill)* |
| **Arrangement** | Guitar cover / tab |
| **Instruments** | Acoustic guitar, electric guitar |

### 📌 Parts I know

- **Electric guitar** *(or acoustic—note which)*
  - Full piece — *(fill in)*
